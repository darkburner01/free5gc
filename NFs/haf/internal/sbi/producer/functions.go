...
func EapEncodeAttribute(attributeType string, data string) (string, error) {
	...
	case "AT_RAND":
		...
		attrNum := fmt.Sprintf("%02x", haf_context.AT_RAND_ATTRIBUTE)
	...
	case "AT_AUTN":
		...
		attrNum := fmt.Sprintf("%02x", haf_context.AT_AUTN_ATTRIBUTE)
		attribute = attrNum + "05" + "0000" + data
	...
	case "AT_KDF":
		...
		attrNum := fmt.Sprintf("%02x", haf_context.AT_KDF_ATTRIBUTE)
		attribute = attrNum + "01" + "0001"

	case "AT_MAC":
		// Pad MAC value with 16 bytes of 0 since this is just for the calculation of MAC
		attrNum := fmt.Sprintf("%02x", haf_context.AT_MAC_ATTRIBUTE)
...
func decodeEapAkaPrime(eapPkt []byte) (*haf_context.EapAkaPrimePkt, error) {
	var decodePkt haf_context.EapAkaPrimePkt
	var attrLen int
	var decodeAttr haf_context.EapAkaPrimeAttribute
	attributes := make(map[uint8]haf_context.EapAkaPrimeAttribute)
	data := eapPkt[5:]
	...
	for i := 3; i < dataLen; i += attrLen {
		...
		switch attrType {
		case haf_context.AT_RES_ATTRIBUTE:
		...
		case haf_context.AT_MAC_ATTRIBUTE:
		...
		case haf_context.AT_KDF_ATTRIBUTE:
			...
			attributes[attrType] = decodeAttr
		case haf_context.AT_AUTS_ATTRIBUTE:
			...
			decodeAttr.Value = data[i+2 : i+attrLen]
			attributes[attrType] = decodeAttr
		case haf_context.AT_CLIENT_ERROR_CODE_ATTRIBUTE:
	...
	case haf_context.AKA_CHALLENGE_SUBTYPE:
		...
		if _, ok := attributes[haf_context.AT_RES_ATTRIBUTE]; !ok {
			return nil, fmt.Errorf("AKA-Challenge attributes error")
		} else if _, ok := attributes[haf_context.AT_MAC_ATTRIBUTE]; !ok {
			return nil, fmt.Errorf("AKA-Challenge attributes error")
		}
	...
	case haf_context.AKA_SYNCHRONIZATION_FAILURE_SUBTYPE:
		...
		} else if _, ok := attributes[haf_context.AT_AUTS_ATTRIBUTE]; !ok {
			return nil, fmt.Errorf("AKA-Synchornization-Failure attributes error")
		} else if _, ok := attributes[haf_context.AT_KDF_ATTRIBUTE]; !ok {
			return nil, fmt.Errorf("AKA-Synchornization-Failure attributes error")
		} else if kdfVal := attributes[haf_context.AT_KDF_ATTRIBUTE].Value; !(kdfVal[0] == 0 && kdfVal[1] == 1) {
			return nil, fmt.Errorf("AKA-Synchornization-Failure attributes error")
		}
	...
	case haf_context.AKA_CLIENT_ERROR_SUBTYPE:
		...
		} else if _, ok := attributes[haf_context.AT_CLIENT_ERROR_CODE_ATTRIBUTE]; !ok {
			return nil, fmt.Errorf("AKA-Client-Error attributes error")
		}
	default:
...
func ConstructFailEapAkaNotification(oldPktId uint8) string {
	...
	eapPkt.Type = haf_context.EAP_AKA_PRIME_TYPENUM

	eapAkaHdrBytes := make([]byte, 3)
	eapAkaHdrBytes[0] = haf_context.AKA_NOTIFICATION_SUBTYPE

	attrNum := fmt.Sprintf("%02x", haf_context.AT_NOTIFICATION_ATTRIBUTE)
...
func getUdmUrl(nrfUri string) string {
	...
	res, err := consumer.SendSearchNFInstances(nrfUri, models.NfType_UDM, models.NfType_HAF, nfDiscoverParam)
	if err != nil {
